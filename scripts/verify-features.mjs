...
//
// The registry sat in packages/shared/src/features.ts for months with nothing reading it,
...
const roles = readFileSync('packages/shared/src/roles.ts', 'utf8')
const features = readFileSync('packages/shared/src/features.ts', 'utf8')
...
if (start === -1) {
  console.error('Could not find SECTION_ACCESS in packages/shared/src/roles.ts.')
  process.exit(1)
}
...
if (orphans.length) {
  ...
  console.error('packages/shared/src/features.ts, or add a feature for it.\n')
}
