// One-off backfill: give every existing game a SKU, and set the counter that
...
// Games are numbered ALPHABETICALLY BY NAME so a printed list sorts the same
...
//
// Safe to re-run. A game that already carries a valid SKU is skipped, never
// renumbered — a SKU is issued once and then belongs to that game, because it
// ends up on shelf labels and past invoices. Re-running only fills in games
...
console.log(`${docs.length} game(s): ${already.length} already have a SKU, ${todo.length} to assign.`)
...
// A collision would mean two games sharing a SKU — refuse rather than write it.
...
for (let i = 0; i < planned.length; i += 400) {
  ...
  for (const p of planned.slice(i, i + 400)) {
    batch.update(db.doc(`games/${p.id}`), { sku: p.newSku })
  }
  await batch.commit()
