...
| 7 | `firestore.rules` | Usually `allow write: if false` (writes go through the route) and a scoped read. If a helper is needed, call `can<Key>()` in a rule and run `npm run rules:generate`: it writes the helper, with the roles from step 1, into the generated block (and removes helpers nothing calls). Never edit the block by hand. A rules deploy is a separate, approved step. | `verify:sections` (the block is what the generator writes); `npm run rules:live` compares with the deployed rules |
