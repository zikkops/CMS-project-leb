...
- [x] **T7.21 A folder for what is sold but is not food** (owner's request,
  24 Sep 2026): game charges, event charges, room hire. There were two kinds
  of thing a check could carry — a menu item, which fires to a kitchen
  station, and a product, which comes off the shelf. A games hour is neither.
  *Done: Done 24 Sep 2026. A fourth section, 'Charges', on the Menu page beside Food, Beverage and Sweets. A category in it is an ordinary folder on the till, so "Games" and "Events" are whatever the cafe wants to call them; its items ring up and are paid for like anything else. What they do NOT do: fire to a station (stationForSection has no entry, and sendCheck already skips a line with none), take a staff-meal rate (staffRateFor already returns 0 rather than guessing), or appear on the Recipes page. The line is stamped `charge: true` at add time from the category's section — NOT read back from "has no station", because a combo's own line has no station either (it fires nowhere; its parts do) and reading it that way would take every combo out of the food cost with it. Absent on every line written before this, which reads as food and is right. The reporting is the part that would have lied: a charge is left out of BOTH sides of the theoretical food cost rather than counted as a dish whose recipe nobody wrote, and in the product mix it is revenue with null cost and null coverage, kept out of the coverage denominator, so a Games row reads as nothing-to-cost instead of 0% covered. Only an exact `true` counts, so a stray value cannot drop real sales out of the figures. Asserted in verify:recipes, verify:reports and verify:checks; 5 mutations caught by name, one of them only after a test was added for the combo trap it guards. Not looked at signed in.*
