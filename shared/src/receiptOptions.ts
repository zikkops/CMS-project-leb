...
//
// Three things print a receipt now: the receipt page, when somebody taps
// Receipt on a closed check; the KDS, when a check closes and the café has
// receipt-on-close switched on; and a café hub printing to a network printer
// (S28). If each built its own options they would drift — a footer changed in
// one, an address in the other — and the reprint a customer asks for would no
// longer match the receipt they were handed. Here, in shared/, because the hub's
// server code is one of the three.
