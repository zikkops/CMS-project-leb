...
import { downloadSalesWorkbook, downloadDaysCsv } from './workbook'
...
export default function SalesExportPage() {
  ...
  async function fetchRange() {
    setBusy('Reading…'); setError(''); setData(null)
    try {
      const params = new URLSearchParams({ from, to, ...(branch ? { branch } : {}) })
      const result = await unwrap(await authedFetch(`/api/admin/exports/sales?${params}`, 'GET'))
      setData(result as unknown as SalesExport)
    } catch (err) {
  ...
  async function toExcel() {
    ...
    try {
      await downloadSalesWorkbook(data, from, to)
    } catch (err) {
